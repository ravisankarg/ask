...
class QueryPlannerSearchOnlyContractTest {
    ...
    fun prompt_declares_model_ownership_and_covers_reported_queries() {
        ...
        assertTrue(prompt.contains("birthday photos from Bangalore excluding cake"))
        assertTrue(prompt.contains("cycling in rain"))
        assertTrue(prompt.contains("beach photos in Chennai"))
        assertTrue(prompt.contains("travel == outside_normal"))
        assertTrue(prompt.contains("are vocabulary, never default filters"))
        assertTrue(prompt.contains("self_person_lookup_only=Ravi"))
        assertTrue(prompt.contains("Never put date words in time"))
        assertTrue(prompt.contains("screenshots containing payment failed"))
